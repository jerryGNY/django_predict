...
def get_categories():
    ...
    channels = GoodsChannel.objects.order_by('group_id', 'sequence') #37条数据
...
def generate_static_index_html():
    ...
    # 1. 获取类别数据
    ordered_dict = get_categories() #有序字典
    # print(ordered_dict)
    ...
    context = {
        'categories': ordered_dict, # {''}  有序字典
        'contents': contents   # 广告内容
    }
    template = loader.get_template('index.html')
    html_text = template.render(context)

    # 4. 把首页内容写到 front_end_pc/index_backup.html 文件中
